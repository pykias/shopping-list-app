...
class ShoppingMainInitClient {
  ...
  async initFinalize() {
    return this.shoppingMainClient.initFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoppingMainInitDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoppingMainInitDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShopping init started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoppingMainInitClient(dtoIn.lockSecret);
  ...
  uuShopping = await mainContext.initFinalize();
