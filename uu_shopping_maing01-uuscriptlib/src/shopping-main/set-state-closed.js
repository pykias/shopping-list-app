...
const Names = {
  ...
  SCRIPT_NAME: "ShoppingMainSetStateClosed",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class ShoppingMainSetStateClosedClient {
  ...
  async setStateClosedFinalize() {
    return this.shoppingMainClient.setStateClosedFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoppingMainSetStateClosedDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoppingMainSetStateClosedDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShopping set state closed started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoppingMainSetStateClosedClient(dtoIn.lockSecret);
  ...
  uuShopping = await mainContext.setStateClosedFinalize();
