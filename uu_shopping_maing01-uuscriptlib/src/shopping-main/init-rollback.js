...
const Names = {
  ...
  SCRIPT_NAME: "ShoppingMainInitRollback",
};
/*@@viewOff:names*/
...
/*@@viewOn:scriptClient*/
class ShoppingMainInitRollbackClient {
  ...
  async initFinalizeRollback() {
    return this.shoppingMainClient.initFinalizeRollback(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptShoppingMainInitRollbackDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptShoppingMainInitRollbackDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuShopping initRollback started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new ShoppingMainInitRollbackClient(dtoIn.lockSecret);
  ...
  uuShopping = await mainContext.initFinalizeRollback();
